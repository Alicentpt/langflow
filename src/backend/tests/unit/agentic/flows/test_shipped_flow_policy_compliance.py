...
registered ``DataFrameKeywordSearch`` component reads its own package source directly. It
...
from lfx.components.processing import component_keyword_search
...
class TestShippedAssistantFlowRunsHardened:
    ...
    async def test_search_component_reads_the_library_with_no_scope_or_exemption(self):
        ...
        await get_and_cache_all_types_dict(get_settings_service())
        graph = await aload_flow_from_json(_prepared(FLOW_PATH), disable_logs=True)
        component = next(v.custom_component for v in graph.vertices if v.id.startswith("DataFrameKeywordSearch"))
...
class TestShippedFlowsStayRegistered:
    ...
    def test_search_node_embeds_the_installed_component_source(self):
        ...
        """
        installed = Path(component_keyword_search.__file__).read_text(encoding="utf-8")
        flow = json.loads(FLOW_PATH.read_text(encoding="utf-8"))
        embedded = next(
            node["data"]["node"]["template"]["code"]["value"]
            for node in flow["data"]["nodes"]
            if node["data"].get("type") == "DataFrameKeywordSearch"
        )
        assert embedded == installed, (
            f"{FLOW_PATH.name} embeds a stale copy of component_keyword_search.py; "
